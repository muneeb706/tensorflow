...
# We will use the average price of a week (7 days) instead of daily prices,
# to train the neural work and predict the average prices for the next five weeks.
# Following functions tests and reports result of the above strategy for the last 12 trading weeks.

def start_trading(consecutive_weeks):
    ...
    predict_weeks = 5
    X, Y = generate_x_y_data(isTest=1, batch_size=consecutive_weeks * 7, predict_days=predict_weeks*7)
    ...
    for j in range(1, consecutive_weeks + 1): 

        past = X[:,(j-1)*7:(j-1)*7 + 7,1]   
        past = past[:predict_weeks * 7]
             
        expected = Y[:,(j-1)*7:(j-1)*7+7,1]
        pred = outputs[:,(j-1)*7:(j-1)*7+7,1]
        
        x0 = np.average(past[-1])
        x1 = np.average(pred[0])
        x2 = np.average(pred[1])
        x3 = np.average(pred[2])
        x4 = np.average(pred[3])
        x5 = np.average(pred[4])
        
        bitcoin_value = P[:,(j-1)*7:(j-1)*7 + 7,1]   
        bitcoin_value = bitcoin_value[:predict_weeks * 7]
        bitcoin_value = np.average(bitcoin_value[-1])
