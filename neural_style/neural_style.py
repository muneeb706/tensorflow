...
Content = directory + 'pitt.jpg'
Style = [directory + 'stars.jpg']
...
def main():
    ...
    if width is not None:
        ...
        content_image = transform.resize(content_image, new_shape)
    target_shape = content_image.shape
    for i in range(len(style_images)):
        style_scale = STYLE_SCALE
        if options.style_scales is not None:
            style_scale = options.style_scales[i]
        style_images[i] = transform.resize(style_images[i], (int(style_scale *
                target_shape[1] / style_images[i].shape[1]), int(style_scale *
    ...
    if initial is not None:
        initial = transform.resize(imread(initial), content_image.shape[:2])
